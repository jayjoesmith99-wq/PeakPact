...
export const getProtocolStatusEffect = (context: ProtocolContext): ProtocolStatusEffect => {
  ...
  if (context.overclockCount >= 2 || context.pp > 300) {
    return 'OVERCLOCK';
  }
...
export const applyRecoveryAction = ({
  ...
  const currentResetDate = usageState.resetDate || resetDate;
...
export const applyStatusEffectToReward = ({
  ...
  baseReward: number;
  effect: ProtocolStatusEffect | string;
}): number => {
  if (effect === 'DRIFT') {
